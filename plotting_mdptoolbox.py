...
def plot_grid_map(column_name, title, grid_values, cur_env):
    ...
    for i in range(grid_values.shape[0]):
        for j in range(grid_values.shape[1]):
            ...
            x = j
            p = plt.Rectangle([x, y], width=1, height=1, facecolor='white', edgecolor='black',
                              linewidth=1.0)
            ax.add_patch(p)

            if column_name == 'policies':
                val_name = 'A'
                val = int(grid_values[y, x])
            elif column_name == 'values':
                val_name = 'V'
                val = np.round(grid_values[y, x], 2)
            elif column_name == 'is_visited':
                val_name = 'Visited'
                val = int(grid_values[y, x])
            text_val = "{}:{}\n{}:{}".format(val_name, val, 'S', str(x))
            text = ax.text(x + 0.5, y + 0.5, text_val, fontsize=font_size_value,
...
def plot_line(df, column_name, legend_name, problem_path, title, x_label, y_label, clear_existing, mark, std):
    ...
    if mark:
        plt.plot(df.index.values, df[cur_column_name], 'o-', linewidth=3, markersize=5,
...
def read_and_plot_map(cur_file_paths, output_dir, cur_file_to_process, cur_env_to_process, cur_prob_to_process,
    ...
    for cur_file_path in cur_file_paths:
        ...
        output_file_name_regex = re.compile(
            '([A-Za-z]+)_([A-Za-z]+)_([A-Za-z]+[=][0-9]+)_([A-Za-z]+)_([0-9A-Za-z]+[-][0-9A-Za-z]+|[0-9]+[.][0-9]+|[A-Za-z]+)_([A-Za-z]+)\.csv')
...
def read_and_plot_line_multi(cur_file_paths, output_dir, cur_file_to_process, cur_env_to_process, cur_prob_to_process):
    for cur_file_path in cur_file_paths:
        ...
        output_file_name_regex = re.compile(
            '([A-Za-z]+)_([A-Za-z]+)_([A-Za-z]+[=][0-9]+)_([A-Za-z]+)_([A-Za-z]+)_([A-Za-z]+)_([A-Za-z]+)\.csv')
